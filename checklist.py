...
with dpg.window(label="Checkbox", width=400, height=760, no_move=True, no_close=True, no_title_bar=True, no_resize=True):
    ...
    for label in LABELS:
        ...
        while len(l) < 90:
            l = f'{l} '
        dpg.add_checkbox(label=l, tag=label)
    with dpg.group(horizontal=True):
        btn_height = 30
        btn_width = 70
        dpg.add_button(label='Start', height=btn_height, width=btn_width, callback=program_timer.start_timer)
        dpg.add_button(label='Pause', height=btn_height, width=btn_width, callback=program_timer.pause_resume_timer, tag=PAUSE_RESUME_TAG)
        dpg.add_button(label='Stop', height=btn_height, width=btn_width, callback=program_timer.stop_timer)
    with dpg.group(horizontal=True):
...
dpg.create_viewport(title='Custom Title', width=400, height=760, resizable=False)
dpg.setup_dearpygui()
dpg.show_viewport()
...
while dpg.is_dearpygui_running():
    ...
    dpg.set_value('elapsed_time', f'{program_timer.display_time(TIME_TYPE_TAG)}')
    dpg.set_value('completion_percentage', f'{num_checked}/{NUM_LABELS}   {round((num_checked/NUM_LABELS)*100, 2)}% Complete.')
